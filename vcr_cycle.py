"""
we need to add some things here first of all here we know how much the refrigerant is providing heat but there will be a resistance considering the the evaporater tube, fins and finally the airs convective resistance so you have to consider that thing into account and maybe say for the time being assume U to be 80 and the area to be 1m2 again the area wil also involve complexities like it will be cross sectional area of fin that is the that of circle with r as radius multiplied by no.of evaporator tubes in each fin and then multiply by two and finally to the the number of total fins in one evaporator and finally multiply with the number of evaporaters like you can comment this as well and assume sastaed above and thus now NTU can be computed using general formula and thus the effectiveness for a cross flow as the relation is known and so we get the heat accepted by air after all the losses note that this heat what we got was by multipying refrigerant heat rejection with effectiveness as stated above but the mass flux of refrigerant was what we were varying so we do not know the exact value but we know how much of air side cooling is the most efficient like the one till the dew point temperature so we know that its the sum of cooling till dew point that si mass flow of air (assume this as like a reasonable value) multiplied with cp of wet air at that particulkar humidity multiplied by temperature difference that is the differnce betweeen Tamb and somewhat near dew point like assume 5 degree below dew point and finally this will be added to latent heat of condensation of water multiplied to amount of water formed that is omega1- omega2 now as earlier see which mass flux of refrigerant is giving that amount of energy as deviation from this will lead to underoptimization as a bonus if you can do you can also check if the flow of air will be turbulent or laminar based on the environment and velocity conditions as this will be the later part of the project whhere we will find convection coeffcient using nusselts number so yea lets see how much can you do i am comparing your results with another claude model lets see who performs the best also in any case do not delete the parts of code like thermodynamics part as that is correct any other queries you can ask also and yea 
