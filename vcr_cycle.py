...
# -------- USER / API CONFIG ----------
# Option A: use wttr.in (no API key needed)
# Provide location: either (lat, lon) tuple or city name string
# Examples:
# LOCATION = ("22.5726", "88.3639")   # lat,lon for Kolkata
# LOCATION = "Kolkata,IN"
location = "Kolkata,IN"  # Change this to your location or use lat, lon like "22.5726,88.3639"

# If fetch fails, the script will ask for manual input (T_amb_C, P_amb_Pa, RH)
# ------------------------------------
...
# Heat exchanger UA (if you want to compute NTU; not strongly used in current energy balance)
UA_evap = 1200.0  # W/K (placeholder, not strongly used in current energy balance)
cp_dry_air = 1005.0  # J/kg-K (approx from psychrolib)
# Dry air specific heat from psychrolib is a function; we'll use 1005 J/kgK if not available
if cp_dry_air is None or cp_dry_air <= 0:
    cp_dry_air = 1005.0
...
def fetch_weather_wttr(location):
    """
    Fetches weather data from wttr.in for a given location.
    Returns temperature and humidity only.
    """
    base = f"https://wttr.in/{location}?format=%t+%h"
    ...
    while attempt < max_retries:
        try:
            response = requests.get(base, timeout=20)  # Increased timeout to 20 seconds
            response.raise_for_status()  # Check if the response is successful (200 OK)
            
            # Parse the response (space-separated temperature and humidity)
            temp, humidity = response.text.split()
            
            # Clean up the temperature string (remove the '+' and '°C' from temperature)
            temp = temp.strip('°C')  # Remove '°C'
            temp = temp.lstrip('+')   # Remove leading '+'
            
            # Now safely convert temperature to float
            T_C = float(temp)  # Temperature in Celsius
            
            # Clean the humidity string (remove '%' sign) and convert to a float
            humidity = humidity.strip('%')  # Remove the '%' sign from humidity
            RH = float(humidity) / 100.0  # Convert percentage to fraction
            
            return {"T_ambient_C": T_C, "RH": RH}
...
def run_simulation(location):
    # 1) fetch ambient
    try:
        weather = fetch_weather_wttr(location)  # Changed function name here to fetch_weather_wttr
        print("Fetched weather from wttr.in.")
    except Exception as e:
        print("wttr.in fetch failed:", e)
    ...
    for m_da in m_vals:
        ...
        if q_evap_specific <= 0:
            ...
            m_ref = Q_dot_air_W / q_evap_specific
            W_comp = m_ref * (h2 - h1)
...
if __name__ == "__main__":
    df_results = run_simulation(location)
    # quick summary head
